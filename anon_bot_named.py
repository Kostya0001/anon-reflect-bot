...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"Новый /start от {user.id} (@{user.username})")
...
async def accept_rules_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"{user.id} (@{user.username}) подтвердил правила")
...
async def handle_all_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    logging.info(f"[{user.id} (@{user.username})] сообщение: {update.message.text.strip()}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.callback_query.from_user
    logging.info(f"Кнопка нажата пользователем {user.id} (@{user.username}): {update.callback_query.data}")
    global answers
    query = update.callback_query
    await query.answer()
